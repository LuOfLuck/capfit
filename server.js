...
// Reutilización centralizada de módulos sin duplicación de lógica
const CONFIG = require('./js/config');
const { readBody, corsHeaders, sendJson, proxyRequest } = require('./api/lib/http-helpers');
const handleGptEdit = require('./api/handlers/gpt-edit');
const handleFalSubmit = require('./api/handlers/fal-submit');
const handleFalStatus = require('./api/handlers/fal-status');
const handleFalResult = require('./api/handlers/fal-result');
const handleAnthropic = require('./api/handlers/anthropic');
...
async function appHandler(req, res) {
  ...
  // ── RUTAS UNIFICADAS DE IA CON CONTROL DE CUOTA Y RATE LIMITING ──
  ...
  // ── Guard para rutas de API no encontradas ───────────────
  // Previene que cualquier ruta no capturada bajo /api/ caiga en el servidor de archivos estáticos y devuelva HTML
  if (reqPath.startsWith('/api/')) {
    sendJson(res, 404, { ok: false, error: `Endpoint de API no encontrado: ${req.method} ${reqPath}` }, req);
    return;
  }
